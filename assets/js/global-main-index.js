 ...
 * Renderização da Página Principal (Dashboard)
 * Versão: 3.0 - Corrigida para Sistema Modular
 * 
 * Funcionalidades:
 * - Renderização de ferramentas em grid
 * - Filtros e busca
 * - Integração com Utils e TemplateEngine
 */
...
(function() {
  ...
  /**
   * Renderiza todas as ferramentas nas seções apropriadas
   */
  ...
  /**
   * Renderiza ferramentas em um container específico
   */
  function renderTools(containerId, typeFilter) {
    const container = document.getElementById(containerId + '-grid');
    if (!container) return;

    // Filtra por tipo
    let filtered = TOOLS_DATA.filter(function(tool) {
    ...
    if (filtered.length > 0) {
      container.innerHTML = filtered.map(function(tool) {
        return Utils.renderCard(tool, state, typeFilter);
      }).join('');
    } else {
      container.innerHTML = 
  ...
  function updateViewButtons() {
    ...
    // Esta função pode ser expandida para mudar visualização
  }
  ...
  function initEventListeners() {
    // Busca de calculadoras
    var calcSearch = document.getElementById('calc-search');
    if (calcSearch) {
    ...
    // Busca de escalas
    var scaleSearch = document.getElementById('scale-search');
    if (scaleSearch) {
