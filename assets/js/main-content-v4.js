...
function renderVisualizar() {
    const visualizarContainer = document.getElementById('visualizar-section');
    if (!visualizarContainer) {
        console.warn('[Main] Elemento #visualizar-section não encontrado');
        return;
    }

    const visualizarHTML = generateVisualizarHTML();
    visualizarContainer.innerHTML = visualizarHTML;
    console.log('[Main] Toolbar de visualização renderizada');
}
...
function renderMainContent() {
    ...
    mainContent.innerHTML = sectionCalculadoras + sectionEscalas + sectionVacinas;
...
// ============================================
function initMain() {
    console.log('[Main] === INICIALIZANDO MAIN CONTENT ===');
    ...
    console.log('[Main] Elementos encontrados:', {
        ...
        visualizarSection: !!visualizarSection
    });

    if (!heroContainer || !mainContent || !visualizarSection) {
...
// Escutar evento do index.html para inicializar após injeção
function setupMainReadyListener() {
    if (window.EventBus && window.EventBus.events && window.EventBus.fired) {
        const firedEvent = window.EventBus.fired["module:main-module-container:ready"];
        if (firedEvent) {
            setTimeout(initMain, 100);
            return;
        }
...
// Fallback: inicialização após delay se o evento não for emitido
function setupFallbackInit() {
    ...
    const checkInterval = setInterval(function() {
        ...
        const heroContainer = document.getElementById('hero-container');
        if (heroContainer && document.getElementById('main-v2-content')) {
        ...
        if (attempts >= maxAttempts) {
            clearInterval(checkInterval);
            console.warn('[Main] Fallback initialization failed after', maxAttempts, 'attempts');
        }
    }, 200);
}
...
// Expor API pública
window.MainContentApp = {
    renderAllTools: renderAllTools,
...
console.log('[Main] main-content.js carregado com sucesso');
