...
class CalculatorSystem {
    ...
    async load() {
        try {
            ...
            // 2. Carregar CSS específico
            await this.loadCSS();
            
            // 3. Carregar HTML da calculadora
            await this.loadHTML();
            
            // 4. Inicializar campos dinâmicos
            this.initializeDynamicFields();
            
            // 5. Inicializar tabs
            this.initializeTabs();
            
            // 6. Inicializar barra lateral de ferramentas
            this.initializeSidebarTools();
            
            // 7. Carregar lógica da calculadora
            ...
            this.dispatchEvent('calculator:loaded', { 
    ...
    async loadConfig() {
        const response = await fetch(`assets/components/${this.calculatorId}.json`);
        ...
        console.log(`📋 Configuração carregada:`, this.config.name);
    }

    async loadCSS() {
        const cssPath = `assets/css/${this.calculatorId}.css`;
        
        try {
            await SystemUtils.loadCSS(cssPath);
            console.log(`🎨 CSS da calculadora carregado: ${cssPath}`);
        } catch (error) {
            console.warn(`⚠️ Não foi possível carregar o CSS: ${cssPath}`, error);
        }
    }

    async loadHTML() {
        const response = await fetch(`assets/components/${this.calculatorId}.html`);
        ...
        if (!container) {
            throw new Error('Container #dynamic-content não encontrado');
        }
            ...
            case 'date':
                inputHTML = `
                    <input id="${field.id}" 
                           ...
                           onchange="SystemUtils.validateDate(this)"/>
                    ${field.placeholder ? `<span class="text-xs text-slate-500 mt-1 block">${field.placeholder}</span>` : ''}
                `;
                break;
    ...
    initializeTabs() {
        ...
        tabsContainer.innerHTML = this.config.tabs.map((tab, index) => `
            <button onclick="window.CALCULATOR_SYSTEM.switchTab('${tab.id}')" 
    ...
    initializeTabContent() {
        ...
        if (sobreContent && this.config.formula) {
            sobreContent.innerHTML = `
                <p class="mb-4">${this.config.description || ''}</p>
                <div class="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 font-mono text-xs overflow-x-auto">
                    <span class="text-nurse-secondary font-bold block mb-2 uppercase tracking-widest">
                        ${this.config.formula.description || 'Equação Auditada'}
                    </span>
    ...
    initializeSidebarTools() {
        ...
        sidebarContainer.innerHTML = this.config.sidebar_tools.map(tool => `
            <button class="tool-btn" 
                    ...
                    tabindex="0"
                    title="${tool.label}" 
                    onclick="window.CALCULATOR_SYSTEM.${tool.action}">
        ...
        // Adicionar container da barra lateral ao DOM se não existir
        if (!document.getElementById('sidebar-tools-container')) {
            const sidebarContainer = document.createElement('aside');
            sidebarContainer.id = 'sidebar-tools-container';
            sidebarContainer.className = 'sidebar-left-tools';
            sidebarContainer.innerHTML = sidebarContainer;
            document.body.appendChild(sidebarContainer);
        }
    }

    async loadCalculatorLogic() {
        const modulePath = `assets/components/${this.calculatorId}.js`;
        
        try {
            // Carregar módulo JS
            await SystemUtils.loadJS(modulePath);
            ...
            if (window.CalculatorLogic && window.CalculatorLogic[this.calculatorId]) {
                this.instance = window.CalculatorLogic[this.calculatorId];
                ...
                console.log(`🧠 Lógica da calculadora carregada: ${this.calculatorId}`);
            } else {
    ...
    loadFallbackLogic() {
        ...
        window.CALCULATOR_SYSTEM = {
            ...
            calculate: function() {
                SystemUtils.showNotification('Funcionalidade de cálculo não implementada', 'warning');
            },
            
            reset: function() {
                document.querySelectorAll('#calculation-fields input, #calculation-fields select').forEach(el => {
                    if (el.type !== 'checkbox') {
                        el.value = '';
                    }
                });
                SystemUtils.showNotification('Campos resetados', 'info');
            },
            
            showTutorial: function() {
                SystemUtils.showModal(
                    'Tutorial - ' + this.config.name,
                    '<p>Tutorial não disponível para esta calculadora.</p>',
                    'fa-question-circle'
                );
            }
        };
        
        this.instance = window.CALCULATOR_SYSTEM;
    }
