/**
 * main-index-v2.js - Main Application Logic
...
function generateHeroHTML() {
    ...
    const slidesHTML = heroSlides.map((slide, index) => `
        <div class="carousel-item ${index === 0 ? 'active' : ''}" data-slide="${index}" style="background-image: ${slide.gradient}, url('${slide.bgImage}'); background-size: cover; background-position: center;">
...
function renderAllTools() {
    ...
    const visualizarContainer = document.getElementById('visualizar-controls');
    const calculatorsGrid = document.getElementById('calculadoras-grid');
    const escalasGrid = document.getElementById('escalas-grid');
    const vacunasGrid = document.getElementById('vacinas-grid');

    if (!heroContainer || !visualizarContainer || !calculadorasGrid) {
        console.warn('[Main] Elementos necessários não encontrados');
        return;
    }
    ...
    // Render Hero
    heroContainer.innerHTML = generateHeroHTML();

    // Render Visualizar Controls
    visualizarContainer.innerHTML = generateVisualizarHTML();

    // Render Grids
    calculatorsGrid.innerHTML = calculators.map(t => Utils.renderCard(t, state, 'calculator')).join('');
    escalasGrid.innerHTML = scales.map(t => Utils.renderCard(t, state, 'scale')).join('');
    vacunasGrid.innerHTML = vaccines.map(t => Utils.renderCard(t, state, 'other')).join('');

    // Apply view mode to all grids
    ...
    console.log('[Main] Conteúdo renderizado com sucesso');
...
function initializeEventListeners() {
    ...
    if (carousel) {
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (carouselRect.top <= window.scrollY && carouselRect.bottom >= window.scrollY) {
                if (e.key === 'ArrowLeft') {
                    prevSlide();
                    startSlideInterval();
...
function updateViewMode() {
    ...
    // Update view icon on button
    if (viewBtn) {
        const iconElement = viewBtn.querySelector('i.fa:not(.fa-chevron-down)');
        if (iconElement) {
            iconElement.className = `fas ${viewIcons[state.viewMode] || 'fa-square'}`;
        }
    }
    ...
    document.querySelectorAll('.view-dropdown .dropdown-item').forEach(item => {
        if (item.dataset.value === state.viewMode) {
            item.classList.add('active');
        } else {
...
function initMain() {
    const mainContent = document.getElementById('main-container');
    if (!mainContent) {
        console.warn('[Main] Elemento #main-container não encontrado, tentando novamente...');
